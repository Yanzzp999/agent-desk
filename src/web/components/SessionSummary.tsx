...
import { EmptyState, formatDateTime, StatusBadge } from "./ui";
...
export function SessionSummary({ sessions }: SessionSummaryProps) {
  return (
    <section className="session-band" aria-label="Recent session summary">
          ...
          {sessions.map((session) => (
            <article key={session.sessionId} className="session-item">
              ...
              <div className="session-meta">
                <span><Clock3 aria-hidden="true" size={15} />{formatDateTime(session.startedAt)}</span>
                <span><Activity aria-hidden="true" size={15} />{session.model} / {session.reasoning} / {session.serviceTier}</span>
