...
AgentDesk 已经有 CLI 入口 `bin/verunectl.mjs`，核心编排逻辑在
...
- `verunectl dev`、`verunectl gui` 和 `verunectl serve` 暴露 GUI/Server 流程
- `src/desktop`、`src/web` 和 `src/server` 实现桌面/Web UI
- `README.md` 仍然记录 GUI/Server 使用方式

CLI 改造前只通过 `verunectl sessions start <taskId> --parallel N` 暴露并发配置，
...
- `package.json` 使用 `bin/verunectl.mjs` 作为包入口，并且只保留 `test` npm script。
- `package.json` 和 `package-lock.json` 已移除 Electron。
- 已删除 `src/desktop`、`src/web` 和 `src/server` 下的 GUI/Web/Server 源文件。
- 已删除 `verunectl dev`、`verunectl gui` 和 `verunectl serve`。
- `verunectl sessions start` 已支持 `--model`、`--reasoning`、`--parallel`、`--concurrency` 和 `--codex-count`。
...
- MCP stdio server 和 `verunectl` 是主要支持的使用入口。
...
2. 运行 CLI help 和命令 smoke check：
   ...
   - `./scripts/verunectl.sh tasks list --project <fixture-project>`
   - `./scripts/verunectl.sh sessions list --project <fixture-project>`
3. 运行 fake Codex 端到端任务生成验证：
   - 将 `CODEX_CLI` 或 `CODEX_CLI_PATH` 指向本地 shim
   - 创建一个带 `master` 分支的临时 git 项目
   - 运行 `verunectl tasks create --title ... --brief ... --project <fixture-project>`
...
- `./scripts/verunectl.sh help`：通过；help 输出只包含 CLI task/session 命令，并记录了 `--model`、`--reasoning`、`--parallel`、`--concurrency` 和 `--codex-count`。
- `./scripts/verunectl.sh tasks list --project /Users/yanzzp/Documents/CodeProjects/agent-desk`：通过，返回 `No AgentDesk tasks found.`
- `./scripts/verunectl.sh sessions list --project /Users/yanzzp/Documents/CodeProjects/agent-desk`：通过，返回 `No AgentDesk sessions found.`
- 确定性 fake Codex E2E：已在 `test/cli-orchestration.test.mjs` 中通过。
  - 创建临时 git 项目并使用 `master` 分支。
  - 通过 fake `CODEX_CLI` 运行 `verunectl tasks create`。
  - 确认生成的 `task.md` 状态变成 `ready`，并包含 3 个子任务。
  - 运行 `verunectl sessions start <taskId> --model gpt-5.5 --reasoning high --parallel 2`。
