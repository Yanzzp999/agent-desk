...
所有内置 skill 都只在显式点名时使用。它们和 MCP stdio server、`verunectl` 一起构成 AgentDesk 的产品路径。AgentDesk 是 headless 的——没有 web UI 或 HTTP API。
...
| `create_agentdesk_task` | 通过 Codex CLI 生成 `.agent-desk/tasks/<taskId>/task.md`；发现相似 task 时先返回候选项，避免重复创建。 |
...
AgentDesk 是 headless 的：没有 web UI 或 HTTP API 需要验证。`npm test` 覆盖控制面、MCP server、CLI 编排以及 Codex/Claude CLI 启动器。
